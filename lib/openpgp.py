...
def _rmtree_error_handler(func, path, exc_info):
    if not isinstance(exc_info[1], OSError) or exc_info[1].errno not in (
        errno.ENOENT,
        errno.ENOTEMPTY,
        errno.EEXIST,
        errno.EBUSY,
...
class OpenPGPSignatureData:
    ...
    def __init__(
        self, fingerprint, timestamp, expire_timestamp, primary_key_fingerprint
    ):
...
"""
    ...
    def close(self):
        if self._home is not None:
            ret, sout, serr = self._spawn_gpg([GNUPGCONF, "--kill", "all"])
            if ret != 0:
                logging.warning(
            ...
            if not self.debug:
                ...
                while os.path.isdir(self._home):
                    shutil.rmtree(self._home, onerror=_rmtree_error_handler)
            else:
                logging.debug(f"GNUPGHOME left for debug purposes: " f"{self._home}")
            self._home = None
    ...
    def _spawn_gpg(self, argv, stdin=b"", **kwargs):
        ...
        if p.wait() != 0:
            raise OpenPGPException(err.decode("utf8", errors="backslashreplace"))
    ...
    def import_key(self, keyfile, trust=True):
        ...
        if trust:
            ...
            exitst, out, err = self._spawn_gpg(
                [GNUPG, "--batch", "--import-ownertrust"], ownertrust
            )
    ...
    def verify_file(self, f):
        ...
        for line in out.splitlines():
            if line.startswith(b"[GNUPG:] GOODSIG"):
                ...
                spl = line.split(b" ", 2)
                if spl[1] in (b"TRUST_MARGINAL", b"TRUST_FULL", b"TRUST_ULTIMATE"):
                    is_trusted = True
    ...
    def clear_sign_file(self, f, outf, keyid=None):
        ...
        exitst, out, err = self._spawn_gpg(
            [GNUPG, "--batch", "--clearsign"] + args, f.read().encode("utf8")
        )
    ...
    def list_keys(self):
        ...
        for l in out.splitlines():
            ...
            if prev_pub is not None:
                ...
                if "@" in addr:
                    logging.debug("refresh_keys_wkd(): UID: {}".format(addr))
                    addrs_key.add(addr)
